        ...
        <div className="min-h-screen bg-secondary/30">
            <main className="container mx-auto max-w-4xl px-4 py-8 pb-24">
                <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                    <Button variant="ghost" onClick={() => router.push('/project-calculator/list')} className="w-full sm:w-auto -ml-4 sm:ml-0">
                        <ArrowLeft className="mr-2 h-4 w-4" />
                        Kembali ke Daftar
                    </Button>
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                         <Button variant="outline" onClick={() => router.push(`/project-calculator?id=${uuid}`)} className="flex-1 sm:flex-initial">
                ...
                <Card className="shadow-lg">
                    <CardHeader>
                        <CardTitle className="text-3xl font-headline break-words">{estimation.title}</CardTitle>
                            ...
                            <div className="space-y-3">
                                {estimation.features.map((feature, index) => (
                                    <div key={index} className="p-4 rounded-lg bg-secondary/50">
                                        <p className="font-semibold break-words">{feature.description}</p>
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1 break-words">{feature.justification}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
